...
'visibilitychange blur mouseleave pagehide resize contextmenu copy paste fullscreenchange webkitfullscreenchange'.split(' ')

  .forEach(e => (window.addEventListener(e, stop, !0), document.addEventListener(e, stop, !0)));
...
#_aw-answer-hint {
  ...
  max-width: min(420px, calc(100vw - 40px)); padding: 8px 12px;

  color: rgba(255,255,255,.72); background: rgba(26,27,30,.62);

  border: 1px solid rgba(255,255,255,.08); border-radius: 8px;

  box-shadow: 0 4px 16px rgba(0,0,0,.18); backdrop-filter: blur(6px);

  font: 12px ui-sans-serif, system-ui, sans-serif; line-height: 1.35;

  opacity: 0; pointer-events: none; transition: opacity .18s ease;

}

#_aw-answer-hint._aw-visible { opacity: .78; }
...
.aw-input { 
  ...
  flex: 1; border: 1px solid rgba(255,255,255,.2); background: rgba(0,0,0,.3); 

  color: #fff; border-radius: 8px; padding: 8px 12px; font-size: 13px; outline: 0; 
...
.aw-btn { 
  ...
  background: linear-gradient(135deg,#a78bfa,#8b5cf6); border: 0; border-radius: 8px; 

  color: #fff; font-weight: 600; padding: 0 16px; cursor: pointer; transition: .2s; 
...
#_aw-toggle-btn { 
  ...
  width: 100%; padding: 8px; font-size: 13px; border-radius: 8px; border: 0; 

  font-weight: 600; cursor: pointer; font-family: inherit; transition: .2s; 

  background: linear-gradient(135deg,#50fa7b,#27c95f); color: #111; 
...
#_aw-color-pick { 
  ...
  width: 32px; height: 24px; border: 1px solid rgba(255,255,255,0.2); 
...
#_aw-color-hex { 
  ...
  width: 72px; border: 1px solid rgba(255,255,255,.2); background: rgba(0,0,0,.3); 

  color: #fff; border-radius: 6px; padding: 4px 6px; font-size: 12px; outline: 0; 
...
#_aw-paste-instruction {

  color: #f59e0b; font-size: 12px; margin-bottom: 8px; text-align: center;
...
#_aw-paste-area {
  ...
  width: 100%; resize: vertical; font-family: monospace; font-size: 11px;
...
let panelEl, statusEl, answerHintEl;
...
const KEY_URL = 'https://raw.githubusercontent.com/Pocimin/wayground-cheat/refs/heads/main/passkey.txt';
...
<div id="_aw-panel">
  ...
  <div id="_aw-paste-instruction">📋 Paste JSON from opened tab below:</div>

  <textarea id="_aw-paste-area" class="aw-input" placeholder="Paste here..." style="display:none"></textarea>
  ...
  <div id="_aw-hint">Ctrl+Shift+Z to hide/show panel · Shift+Z to toggle answers</div>
  ...
  answerHintEl = document.createElement('div');
  answerHintEl.id = '_aw-answer-hint';
  document.body.appendChild(answerHintEl);
  ...
  const pasteInstr = $('#_aw-paste-instruction');

  const pasteArea  = $('#_aw-paste-area');
  ...
  const processJsonInput = () => {
    ...
    try {
      ...
      processAnswerData(parsed);

      

      pinRow.style.display = 'none';

      pasteInstr.style.display = 'none';
      ...
      console.error('[AW] Parse error:', err);

      statusEl.textContent = '❌ Invalid JSON - try again';
  ...
  const handlePinLoad = async () => {
    ...
    const apiUrl = `https://api.cheatnetwork.eu/quizizz/${pin}/answers`;

    try {

      const res = await _fetch(apiUrl, { headers: { Accept: 'application/json' } });

      if (!res.ok) throw new Error(`HTTP ${res.status}`);

      processAnswerData(await res.json());
      pinRow.style.display = 'none';
      toggleRow.style.display = 'block';
      statusEl.textContent = 'Ready - watching for questions';
      statusEl.style.color = '#50fa7b';
      ...
      console.warn('[AW] Automatic load failed; using paste fallback:', err);
    ...
    pasteInstr.style.display = 'block';

    pasteArea.style.display = 'block';

    statusEl.textContent = '📋 Copy ALL text from tab, paste below';

    statusEl.style.color = '#f59e0b';

    

    setTimeout(() => pasteArea.focus(), 300);
  ...
  pasteArea.addEventListener('paste', (e) => {
    ...
    setTimeout(processJsonInput, 150);
...
function findGamePin() {
  ...
  const matches = text.match(/\b\d{3,4}[\s-]*\d{4}\b/g) || [];

  return matches.map(value => value.replace(/\D/g, '')).find(pin => pin.length === 7 || pin.length === 8) || null;
...
function processAnswerData(parsed) {

  console.log('[AW] Processing data:', parsed);

  

  const questions = parsed?.answers || [];
  ...
  questions.forEach((q) => {
    ...
    const ansRaw = q?.answer !== undefined ? q?.answer : q?.structure?.answer;

    const opts = q?.options || q?.structure?.options || [];
    ...
    const answers = indices.map(idx => {
      ...
      const text = opt?.text || opt?.content || opt;
    ...
    console.log('[AW] Question:', id, '→', answers);
  ...
  console.log('[AW] Total loaded:', cache.size, 'questions');
...
function getQuestion() {
  ...
  const options = $$('.option.is-selectable').map(el => {
    ...
    const textEl = el.querySelector('.option-text-inner, .text-container, div.content-slot') || el;
...
function clearHighlights() {
  ...
  answerHintEl?.classList.remove('_aw-visible');
  if (answerHintEl) answerHintEl.textContent = '';

}

function showAnswerHint(q, ans) {

  if (!answerHintEl) return;

  const targets = typeof ans === 'string' && ans.includes('Open-ended')
    ? ['Open-ended']
    : Array.isArray(ans) ? ans : [ans];

  const labels = targets.map(target => {

    const opt = q.options.find(o => o.text === target);

    return opt?.element.innerText?.replace(/\s+/g, ' ').trim() || target;

  });

  if (labels.length) {

    answerHintEl.textContent = `Answer: ${labels.join(' • ')}`;
    answerHintEl.classList.add('_aw-visible');
...
function highlight() {
  ...
  if (!cache.size || !answersVisible) return;
  ...
  if (typeof ans === 'string' && ans.includes('Open-ended')) {

    showAnswerHint(q, ans);
    return;
  ...
  console.log('[AW] Looking for:', targets);

  console.log('[AW] Available options:', q.options.map(o => o.text));
  ...
  targets.forEach(target => {
    ...
    if (opt) {
      ...
      console.log('[AW] ✅ Highlighting:', target);
...
console.log('[AW] Loaded — Ctrl+Shift+Z to toggle panel · Shift+Z to toggle answers');
